...
def convert_to_range(numbers):
    ...
    For example: 1,2,3,6,7,8,9,12,23,24 will become 1-3,6-9,12,23-24
    """
    range = list()
...
def print_info(job, info):
    """
    Print the information in the job structure for the requested info items and reformat the data

    The info string is a comma-separated list with items of the form input_key:output_key. The input_key
    ...
    for (input_key, output_key) in keys:
        ...
        try:
            print "%s: %s" % (output_key, transformers[input_key](value))
        except KeyError:
            print "%s: %s" % (output_key, value)
...
def main():
    ...
    options = {
        ...
        "information": ("Comma-separated list of the job info to print", None, "store", None),
    }
    opts = simple_option(options)
    pquery = PBSQuery()
    current_job = pquery.getjob(opts.options.jobid)

    print_info(current_job, opts.options.information)
